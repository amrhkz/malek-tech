...
const ThemeDropdown = () => {
  ...
  const getIcon = () => {
    if (theme === "light") return <i className="bx bxs-sun bx-sm"></i>;
    if (theme === "dark") return <i className="bx bxs-moon bx-sm"></i>;
    return <i className="bx bx-adjust bx-sm"></i>;
  };

  return (
    <div className="nav-item dropdown dark-toggle" ref={dropdownRef}>
      <div className="flex items-center justify-center" onClick={() => setIsOpen(!isOpen)}>{getIcon()}</div>
      {isOpen && (
        <div className="theme-dropdown open">
          ...
          >
            <i className="bx bxs-sun bx-xs"></i> Light
          </button>
          <button
            className={theme === "dark" ? "active" : ""}
            onClick={() => changeTheme("dark")}
          >
            <i className="bx bxs-moon bx-xs"></i> Dark
          </button>
          <button
            className={theme === "system" ? "active" : ""}
            onClick={() => changeTheme("system")}
          >
            <i className="bx bxs-moon-phase-1 bx-xs"></i> System
          </button>
        </div>
      )}
    </div>
